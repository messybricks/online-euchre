...
/**
 * a window listener specifically to make it so that the server exits properly
 * when closed with the close button
 * 
 * @author bmsmolin
 * 
 */
public class ServerWindowListener implements WindowListener
{
	...
	@Override
	/**
	 * the method that is called when the close button is hit, does the exact
	 * same thing as the exit command from terminal
	 */
	public void windowClosing(WindowEvent event)
	{
		if (thread != null && !thread.disposed())
		{
			try
			{
